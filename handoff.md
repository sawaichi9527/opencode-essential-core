...
| 項目 | 值 |
|---|---|
| 版本 | `2.0.14`（`VERSION` / `manifest/skills.json`；基於 OpenCode v2.0.14 驗證） |
| 分支 | `main`（2.0.14：新增 5 個 user-invocable commands；HEAD 以 `git log -1` 為準；PR #3 已關閉由 #4 取代、PR #4 已合併） |
...
- `commands/` — 六個手動 command：`project-init.md`、`environment-check.md`、`config-check.md`、`session-start.md`、`session-close.md`、`teamwork-update-check.md`（安裝至 `~/.config/opencode/commands/`；前五個是載入同名 Skill 的薄包裝，唯讀者用 `agent: plan`）
...
- `manifest/skills.json` — 機器可讀元件清單，schemaVersion 1，version 2.0.14
...
1. **2.0.14（9/23）**：新增 5 個 user-invocable command（`/project-init`、`/environment-check`、`/config-check`、`/session-start`、`/session-close`），薄包裝載入同名 Skill，讓原本只有 Skill 形式的流程可由使用者直接叫用（`/project-init` 與內建 `/init` 明確區分）；唯讀者用 `agent: plan`。同步更新 `scripts/check.*`（期待 6 個 command）、manifest 元件、README Commands 段與目錄樹、CHANGELOG。另延續 2.0.13-dev：文件標明 OpenCode v2 完整 Skill discovery 目錄（原生 `~/.config/opencode/skills/`、`.opencode/skills/` 與相容 `~/.agents/skills/`、`.agents/skills/`），並移除 README 已失效的 `hybrid-workflow` 描述。版號改為 `2.0.14`。既有安裝可透過 `/teamwork-update-check` 取得這 5 個新 command（manifest 新元件 → `ADDED`；`teamwork-update-check` SKILL 的 baseline 範例與 README 已同步）。
