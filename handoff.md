...
- Eight Core Skills、two Core Commands（`/teamwork-update-check`、`/instructions`）安裝至 `~/.config/opencode/skills/` 與 `~/.config/opencode/commands/`（v2.x.x）；v1.x.x 的命令目錄為 `~/.config/opencode/command/`。
